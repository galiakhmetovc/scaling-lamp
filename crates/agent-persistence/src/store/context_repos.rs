    ...
    fn get_context_summary(
        ...
        self.with_client(|client| {
            client
                ...
                .map(|row| row.map(|row| context_summary_from_row(&row)))
            ...
            client.execute(
                "INSERT INTO context_offloads (session_id, refs_json, updated_at)
                 ...
                 ON CONFLICT(session_id) DO UPDATE SET
                    ...
                    updated_at = excluded.updated_at",
                &[&record.session_id, &record.refs_json, &record.updated_at],
            )?;
            Ok(())
        })?;
    ...
    fn get_context_offload(
        ...
        self.with_client(|client| {
            client
                ...
                .map(|row| row.map(|row| context_offload_from_row(&row)))
                .map_err(StoreError::from)
        })
    }
            ...
            client.execute(
                "INSERT INTO plans (session_id, items_json, updated_at)
                 ...
                 ON CONFLICT(session_id) DO UPDATE SET
                    ...
                    updated_at = excluded.updated_at",
                &[&record.session_id, &record.items_json, &record.updated_at],
            )?;
            Ok(())
        })
    }
    ...
    fn get_plan(&self, session_id: &str) -> Result<Option<PlanRecord>, StoreError> {
        self.with_client(|client| {
            client
                ...
                .map(|row| row.map(|row| plan_from_row(&row)))
                .map_err(StoreError::from)
        })
    }
}
    ...
    fn get_artifact(&self, id: &str) -> Result<Option<ArtifactRecord>, StoreError> {
        ...
        match row {
            Some((
                ...
                let path = self.layout.root_dir.join(&stored_path);
                ...
                Ok(Some(ArtifactRecord {
                    ...
                    metadata_json,
                    path: PathBuf::from(
                        path.strip_prefix(&self.layout.root_dir)
...
impl PersistenceStore {
    fn query_context_summaries(
        &self,
        sql: &str,
        params: &[&(dyn ToSql + Sync)],
    ) -> Result<Vec<ContextSummaryRecord>, StoreError> {
        self.with_client(|client| {
            client
                .query(sql, params)
                .map(|rows| rows.iter().map(context_summary_from_row).collect())
                .map_err(StoreError::from)
        })
    }
}

fn context_summary_from_row(row: &Row) -> ContextSummaryRecord {
...
fn context_offload_from_row(row: &Row) -> ContextOffloadRecord {
...
fn plan_from_row(row: &Row) -> PlanRecord {
    PlanRecord {
        session_id: row.get(0),
