use agent_persistence::{
    ...
    FileDeliveryRequestRecord, SessionInboxRepository, TaskRegistryRecord, TaskRegistryRepository,
    TelegramRepository, TelegramUserPairingRecord, TranscriptRecord, TranscriptRepository,
};
...
#[test]
fn telegram_worker_renders_delegated_task_details() {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .worker_threads(8)
        .enable_all()
        .build()
        .expect("runtime");
    let (_temp, app) = telegram_test_app();
    seed_activated_pairing(&app, "pair-task", 777, 42);
    let session = app
        .create_session_auto(Some("Task owner"))
    ...
    assert!(task_response.body.contains("task-agent-1"));
...
fn telegram_worker_processes_status_and_stop_while_chat_turn_is_running() {
    ...
    let first_poll = runtime.block_on(async {
        tokio::time::timeout(Duration::from_secs(10), worker.poll_once()).await
    });
    ...
    *released.lock().expect("release lock") = true;
    ...
    assert!(
        captured
            .iter()
            .any(|request| request.body.contains("Current session"))
    );
    assert!(
        captured
            .iter()
            .any(|request| request.body.contains("stopped session-running"))
    );
    assert!(
        captured
            .iter()
            .any(|request| request.body.contains("cancelled session-running"))
    );
    assert!(
        captured
            .iter()
            .any(|request| request.body.contains("Long turn completed."))
    );
    let status_record = app
        .store()
        .expect("open store")
        .get_telegram_chat_status(42)
        .expect("status lookup")
        .expect("status retained until TTL cleanup");
    assert_eq!(status_record.state, "stale");
...
fn telegram_worker_coalesces_inbound_messages_while_turn_is_running() {
    ...
    runtime
        .block_on(async { tokio::time::timeout(Duration::from_secs(10), worker.poll_once()).await })
    ...
    started_receiver
        .recv_timeout(Duration::from_secs(2))
        .expect("chat turn started");
    drive_runtime(&runtime, Duration::from_millis(200));
    ...
    *released.lock().expect("release lock") = true;
    ...
    let status_record = app
        ...
        .get_telegram_chat_status(42)
        .expect("status lookup")
        .expect("status retained until next input or TTL cleanup");
    assert_eq!(status_record.state, "stale");
...
fn telegram_worker_registers_current_chat_target_and_attaches_session_output() {
    ...
    let (api_url, requests, handle) = spawn_fake_telegram_api(vec![
        json_response(
            r#"{"ok":true,"result":[
                ...
                {"update_id":152,"message":{"message_id":120,"date":0,"chat":{"id":42,"type":"private"},"from":{"id":777,"is_bot":false,"first_name":"Alice","username":"alice"},"text":"/targets"}},
                {"update_id":153,"message":{"message_id":121,"date":0,"chat":{"id":42,"type":"private"},"from":{"id":777,"is_bot":false,"first_name":"Alice","username":"alice"},"text":"/outputs attach telegram-n9000 summary"}},
                {"update_id":154,"message":{"message_id":122,"date":0,"chat":{"id":42,"type":"private"},"from":{"id":777,"is_bot":false,"first_name":"Alice","username":"alice"},"text":"/routes"}},
                {"update_id":155,"message":{"message_id":123,"date":0,"chat":{"id":42,"type":"private"},"from":{"id":777,"is_bot":false,"first_name":"Alice","username":"alice"},"text":"/outputs detach telegram-n9000"}},
                {"update_id":156,"message":{"message_id":124,"date":0,"chat":{"id":42,"type":"private"},"from":{"id":777,"is_bot":false,"first_name":"Alice","username":"alice"},"text":"/routes"}}
            ]}"#,
        ),
        json_response(
            r#"{"ok":true,"result":{"message_id":125,"date":0,"chat":{"id":-9000,"type":"group"},"text":"target"}}"#,
        ),
        json_response(
            r#"{"ok":true,"result":{"message_id":126,"date":0,"chat":{"id":42,"type":"private"},"text":"targets"}}"#,
        ),
        json_response(
            r#"{"ok":true,"result":{"message_id":127,"date":0,"chat":{"id":42,"type":"private"},"text":"outputs"}}"#,
        ),
        json_response(
            r#"{"ok":true,"result":{"message_id":128,"date":0,"chat":{"id":42,"type":"private"},"text":"routes"}}"#,
        ),
        json_response(
            r#"{"ok":true,"result":{"message_id":129,"date":0,"chat":{"id":42,"type":"private"},"text":"detach"}}"#,
        ),
        json_response(
            r#"{"ok":true,"result":{"message_id":130,"date":0,"chat":{"id":42,"type":"private"},"text":"routes-empty"}}"#,
        ),
    ]);
    let client = TelegramClient::new(TelegramClientConfig {
        token: "test-token".to_string(),
    ...
    let detached_route = store
        .get_session_output_route(&format!("route-{}-telegram-n9000", session.id))
        .expect("get detached route")
        .expect("route exists");
    assert!(!detached_route.enabled);
    assert_eq!(detached_route.target_id, "telegram-n9000");
    assert_eq!(detached_route.format_policy, "summary");
    ...
    let targets_response = requests
        .recv_timeout(Duration::from_secs(2))
        .expect("captured targets response");
    assert!(targets_response.body.contains("Delivery targets"));
    assert!(targets_response.body.contains("telegram-n9000"));
    let outputs_response = requests
        .recv_timeout(Duration::from_secs(2))
        .expect("captured outputs response");
    assert!(outputs_response.body.contains("attached"));
    let routes_response = requests
        .recv_timeout(Duration::from_secs(2))
        .expect("captured routes response");
    assert!(routes_response.body.contains("Session outputs"));
    assert!(routes_response.body.contains("telegram-n9000"));
    let detach_response = requests
        .recv_timeout(Duration::from_secs(2))
        .expect("captured detach response");
    assert!(detach_response.body.contains("detached"));
    let routes_empty_response = requests
        .recv_timeout(Duration::from_secs(2))
        .expect("captured empty routes response");
    assert!(routes_empty_response.body.contains("<empty>"));
...
fn telegram_worker_caches_bot_identity_across_group_mentions() {
    ...
    app.create_session("session-group-1", "Group Chat")
    ...
    app.store()
        ...
        .put_telegram_chat_binding(&agent_persistence::TelegramChatBindingRecord {
            ...
            scope: "group".to_string(),
            owner_telegram_user_id: None,
            selected_session_id: Some("session-group-1".to_string()),
            default_agent_profile_id: None,
            last_delivered_transcript_created_at: Some(0),
            last_delivered_transcript_id: Some(String::new()),
            inbound_queue_mode: "coalesce".to_string(),
    ...
    let backend = RecordingTelegramBackend::with_state(RecordingTelegramBackendState {
        session_lookup: BTreeMap::from([(
            "session-group-1".to_string(),
            session_summary("session-group-1", "Group Chat", false),
        )]),
        next_chat_output: "Cached group reply.".to_string(),
        ..RecordingTelegramBackendState::default()
    });
    let backend_state = backend.state();
    let mut update_responses = VecDeque::from([r#"{"ok":true,"result":[
    ...
    let (api_url, requests, handle) = spawn_routed_telegram_api(move |request| {
        ...
        if request.path.ends_with("/GetMe") {
            return json_response(
                r#"{"ok":true,"result":{"id":9001,"is_bot":true,"first_name":"teamd","username":"teamd_agent_bot","can_join_groups":true,"can_read_all_group_messages":false,"supports_inline_queries":false,"has_main_web_app":false}}"#,
            );
        }
        if request.path.ends_with("/SendMessage") || request.path.ends_with("/EditMessageText") {
            return json_response(
                r#"{"ok":true,"result":{"message_id":42,"date":0,"chat":{"id":9000,"type":"group","title":"Team Chat"},"text":"ok"}}"#,
            );
        }
        json_response(r#"{"ok":true,"result":true}"#)
    });
    ...
    assert_eq!(
        state.executed_turns,
        vec![("session-group-1".to_string(), "first".to_string())]
    );
    drop(state);
    ...
    assert_eq!(
        captured
            .iter()
            .filter(|request| request.path == "/bottest-token/GetMe")
            .count(),
        1,
        "bot identity should be fetched once and then cached: {captured:?}"
    );
    assert!(
        captured.iter().any(|request| {
        ...
        "expected first temporary status: {captured:?}"
    );
    assert!(
        captured.iter().any(|request| {
            request.path == "/bottest-token/SendMessage"
                && request.body.contains("Cached group reply.")
        }),
        "expected first final response: {captured:?}"
    );
    assert!(
        captured.iter().any(|request| {
            request.path == "/bottest-token/SendMessage"
        ...
        "expected active-turn response for second mention: {captured:?}"
    );
