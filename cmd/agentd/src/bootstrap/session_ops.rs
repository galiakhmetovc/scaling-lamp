...
impl App {
    ...
    #[cfg_attr(not(test), allow(dead_code))]
    pub fn list_session_summaries_for_agent_page(
        &self,
        ...
        DiagnosticEventBuilder::new(
            ...
            "info",
            "session_ops",
            "list_session_summaries_for_agent_page.start",
            "listing paginated session summaries for agent",
        )
        .field("agent_profile_id", agent_profile_id)
        .field("limit", limit)
        .field("offset", offset)
        .emit(&self.persistence.audit);
        let store = self.store()?;
        let records = store.list_sessions_for_agent_page(agent_profile_id, limit, offset)?;
        let summaries = build_session_summaries_for_records(
            &store,
            &self.config,
            &self.runtime.workspace,
        ...
        DiagnosticEventBuilder::new(
            ...
            "info",
            "session_ops",
            "list_session_summaries_for_agent_page.finish",
            "listed paginated session summaries for agent",
        )
        .elapsed_ms(started.elapsed().as_millis() as u64)
        .outcome("ok")
        .field("agent_profile_id", agent_profile_id)
