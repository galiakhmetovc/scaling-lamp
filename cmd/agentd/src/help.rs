...
fn render_overview_help() -> String {
    [
        ...
        "- \\помощь судья — как использовать локальный профиль `judge`, если вы его создали",
        "",
        "Коротко:",
        "- основные команды на русском; английские /... алиасы пока сохранены",
        "- Shift+Tab перебирает команды и подставляет их в поле ввода",
        "- встроенный шаблон только один: `default`; остальные агенты создаются как отдельные workspace-профили",
...
fn render_commands_help() -> String {
    [
        ...
        "- \\агент создать <имя> — создать durable agent profile из шаблона default",
        "- \\агент открыть [id|name] — показать agent workspace, prompt files, skills_dir и default workspace",
        ...
        "- \\судья <сообщение> — быстрый алиас для отправки сообщения агенту с id `judge`, если такой профиль создан",
        ...
        "- \\фоновые — показать активные runtime/background jobs текущей сессии",
        "- \\задачи — открыть delegated/MIMO task registry текущей сессии",
        "- \\задача <id> — открыть карточку delegated task; \\задача отменить <id> отменяет её",
        ...
        "",
        "Алиасы:",
        "- английские /help, /context, /jobs, /tasks, /task, /completion, /autoapprove и другие пока работают",
    ]
    .join("\n")
}
...
fn render_judge_help() -> String {
    [
        "Судья",
        "",
        "`judge` больше не встроенный шаблон. Это обычный agent profile, который можно создать из `default` и ограничить allowlist тулов.",
        "Как включить:",
        "- \\агент создать judge",
        "- \\агент выбрать judge",
        "- \\новая",
        "",
        "Что это даёт:",
        "- новая сессия возьмёт SYSTEM.md, AGENTS.md и skills из workspace агента `judge`",
        "- read-only поведение теперь задаётся настройками профиля и allowlist тулов, а не скрытым шаблоном",
