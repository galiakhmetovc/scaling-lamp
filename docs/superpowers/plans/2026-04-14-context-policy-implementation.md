...
**Goal:** Replace the entire legacy context/runtime behavior layer with a clean-room agent architecture where all non-essential behavior is optional, strategy-based, merged into resolved contracts, and applied consistently by transport, request shape, prompt assembly, memory handling, tool execution, observability, and web display.

**Architecture:** Treat the current system as legacy reference only and replace it bottom-up. Use an event-sourced runtime core with projections for operational reads. Keep canonical data objects (`SessionHead`, `WorkspacePointer`, `ArtifactRegistry`, `Transcript`, `Plan`) separate from behavior policy families. Resolve policy families into runtime contracts: `ProviderRequestContract`, `MemoryContract`, `ExecutionContract`, `DisplayContract`, and `ObservabilityContract`. Apply those contracts through dedicated executors instead of scattered conditionals. Start from the bottom of the stack with transport/request delivery policy, then move upward into prompt, memory, tools, observability, and web surfaces until legacy paths can be removed.
...
- tool gate/executor
- display adapter
- observability and trace adapter

## Accepted Rewrite Constraints

These constraints are already decided and should not be reopened during implementation:

1. The rewrite is clean-room; legacy code is reference only.
2. No compatibility hacks, temporary simplifications, or "start smaller first" shortcuts.
3. One binary, one root config path, one agent instance.
4. More agents are separate process launches with separate config files.
5. Configuration is modular and explicit.
6. Contracts and policies live in separate files.
7. Contracts reference policy modules by path; every module has its own stable `id`.
8. No `extends`, no hidden imports, no implicit multi-agent config graph.
9. Initial implementation stays inside one compiled binary with built-in registries.
10. Prompt assets are not free-form config blobs; they belong to their own policy-governed domain.
...
- Create: `internal/runtime/events.go`
  Responsibility: canonical domain event model
- Create: `internal/runtime/event_log.go`
  Responsibility: append-only event storage contract
- Create: `internal/runtime/projections/`
  Responsibility: session, session-head, plan, workspace-pointer, and run projections
...
- Create: `internal/config/loader.go`
  Responsibility: load explicit root config and module graph
- Create: `internal/config/module_registry.go`
  Responsibility: register and validate config module kinds
- Create: `internal/runtime/agent_builder.go`
  Responsibility: build one agent instance from one root config path
- Create: `config/agent.yaml`
  Responsibility: root explicit agent composition manifest
- Create: `config/contracts/`
  Responsibility: contract module files
- Create: `config/policies/`
  Responsibility: policy module files
...
- explicit transport policy modules for endpoint/auth/retry/timeout/tls/rate-limit

#### Accepted first transport baseline

For `z.ai`, the first implemented baseline is:
...
`TLSPolicy` and `RateLimitPolicy` are part of the contract from the start even if initial executor support is partial.
...
Replace:
- ad hoc raw request and response tracing
- scattered trace capture and UI-only debug plumbing
