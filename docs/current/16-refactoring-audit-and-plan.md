...
### 6. Legacy надо удалять после явного runtime решения

В 1.2.0 уже удалены старый embedded store, migration CLI и short filesystem tool ids из `ToolCatalog`. Оставшийся cleanup должен идти по тем же правилам:
...
Статус: выполнено для filesystem tool surface и storage docs. Сквозная модель добавлена в [17-runtime-mental-model.md](17-runtime-mental-model.md). SilverBullet описан как canonical knowledge add-on.

Работы:

- убрать старые aliases из `ToolCatalog`;
- убрать старые add-on paths из основных happy-path инструкций;
...
```bash
rg -n "legacy|unsupported add-on|deprecated" docs/current crates cmd
```
