...
Artifacts также используются для файлов: Telegram uploads сохраняются как `telegram_file`, workspace files для отправки пользователю материализуются как `workspace_file`, а `deliver_file` создаёт запись в `file_delivery_requests`, чтобы surface доставила файл без отдельного Telegram-specific tool path.
