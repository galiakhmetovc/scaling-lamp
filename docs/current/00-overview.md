...
Текущий `agent_home` живёт в `data_dir/agents/<agent_id>/` и содержит `SYSTEM.md`, `AGENTS.md` и `skills/`. Это editable profile home, а не project workspace. Рабочий каталог теперь хранится на уровне session как persisted `workspace_root`. План дальнейшего разделения `agent templates`, `agent profiles` и рабочих директорий описан в [11-workspace-modernization-plan.md](11-workspace-modernization-plan.md).
