...
- после `message_agent`, только если нужен реальный ответ другого агента до завершения текущего хода;
- для bounded диагностики чужой session с явным timeout.

Когда не использовать:

- если достаточно продолжить текущую работу и посмотреть `/tasks`/task registry позже;
- как скрытое ожидание внутри transport/router flow.
...
- возвращает ids дочерней ветки/task flow сразу;
...
- нормальный flow — продолжить работу и смотреть `/tasks`/task registry позже;
- если нужен ответ до завершения текущего хода, используйте `session_wait` по возвращённому `recipient_session_id` с bounded timeout.
