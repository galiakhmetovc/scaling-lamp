...
Если нужен настоящий JS-capable browser, он подключается как optional MCP connector, например Lightpanda. Это не новый built-in `browser_*` tool surface: модель получает discovered MCP tools через тот же `automatic_provider_tools(...)`, а все вызовы проходят через общий provider loop, tool ledger, artifacts/offload и debug UI.
