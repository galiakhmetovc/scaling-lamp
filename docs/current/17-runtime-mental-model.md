...
Mem0 нельзя использовать как скрытый KV. Для точных ключей, блокировок, счётчиков и runtime-очередей нужен `agent-persistence` или отдельный KV-слой; Mem0 отвечает только за семантический поиск по durable facts.
