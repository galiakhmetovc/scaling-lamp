...
Mem0 не подмешивается в prompt скрыто. Агент должен явно вызвать `memory_search` или `memory_list`, увидеть результат в tool ledger и только потом использовать его в ответе. Optional post-turn `memory_curator` может записывать durable facts автоматически после ответа, но чтение памяти всё равно остаётся явным и inspectable.
