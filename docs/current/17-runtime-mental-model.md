...
Mem0 не подмешивается в prompt скрыто. Агент должен явно вызвать `memory_search` или `memory_list`, увидеть результат в tool ledger и только потом использовать его в ответе. Скрытая “магическая память модели” не считается источником истины.
