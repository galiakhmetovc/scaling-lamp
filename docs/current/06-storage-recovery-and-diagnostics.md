...
То есть состояние — это не “только PostgreSQL”. PostgreSQL хранит control-plane metadata и индексы, а большие тела лежат рядом на файловой системе как payload-файлы. Старый `state.sqlite` может существовать только как legacy source для миграции/rollback, но runtime больше не открывает его как каноническую БД.
...
| `state.sqlite` | Legacy SQLite file из старых установок. Runtime больше не использует его как canonical store; читать его нужно только через `agentd migrate sqlite-to-postgres` или ручной rollback-процесс. | Нет. |
...
## Миграция legacy SQLite в PostgreSQL

Runtime больше не открывает `state.sqlite` как canonical store. Для обновления старого узла используется явная команда миграции:

```bash
agentd migrate sqlite-to-postgres \
  --sqlite /var/lib/teamd/state/state.sqlite \
  --database-url 'postgresql://teamd:password@127.0.0.1:5432/teamd' \
  --data-dir /var/lib/teamd/state
```

Требования:

- target PostgreSQL database/schema должен быть пустым для runtime tables;
- payload directories (`artifacts/`, `transcripts/`, `archives/`, `runs/`) остаются на месте и должны быть доступны новому runtime;
- перед миграцией остановите `teamd-daemon.service` и `teamd-telegram.service`;
- после миграции задайте `TEAMD_DATABASE_URL` в `/etc/teamd/teamd.env` и запустите services.

Команда копирует metadata/control-plane rows в PostgreSQL. Она не дублирует большие payload-файлы, потому они уже лежат рядом в `data_dir`.
