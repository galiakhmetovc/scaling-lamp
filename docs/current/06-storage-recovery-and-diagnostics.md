...
| --- | --- | --- |
| `agent-templates/default/SYSTEM.md` | Runtime-editable default template system prompt. Используется для bootstrap default profile и создания новых agent workspaces. Если файла нет, daemon создаёт его из bundled repo template. | Да. Правка влияет на будущий bootstrap/sync default profile и новые агенты, но не обязана молча затирать вручную изменённые custom workspaces. |
| `agent-templates/default/AGENTS.md` | Runtime-editable default template tool/profile guidance. | Да. Используйте для изменения default template без пересборки `agentd`. |
| `agent-templates/default/skills/` | Runtime-editable default template skills. | Да. Это источник shipped skills для default profile и новых агентов. |
...
| `../workspaces/agents/<agent_id>/SYSTEM.md` | System prompt конкретного agent profile. Runtime читает его при сборке prompt. | Да, осознанно. Влияет на будущие turns этого agent profile. |
| `../workspaces/agents/<agent_id>/AGENTS.md` | Инструкции и tool-usage guidance конкретного agent profile. | Да, осознанно. Влияет на будущие turns. |
| `../workspaces/agents/<agent_id>/skills/` | Skills конкретного agent profile. Полная папка skill принадлежит workspace агента. | Да, если понимаете формат skills. |
| `agents/<agent_id>/` | Legacy layout старых версий. Bootstrap копирует оттуда missing prompt/skill files в `../workspaces/agents/<agent_id>/`. Runtime больше не читает skills отсюда как primary source. | Не редактировать, мигрировать в workspace. |
...
Важно: canonical место профиля теперь `workspaces/agents/<agent_id>/`. Это одновременно workspace агента, место его `SYSTEM.md`/`AGENTS.md`/`skills/` и default рабочая директория новых session. Старое поле `agent_profiles.agent_home` в БД сохраняется как compatibility field, но должно указывать на этот workspace.
...
| `agent_profiles` | Agent profiles, template kind, allowed tools, путь к canonical agent workspace. Поле `agent_home` в схеме пока compatibility name. |
