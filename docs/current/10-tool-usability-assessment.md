...
### 3. Short filesystem tool names удалены из каталога

В 1.2.0 runtime перестал публиковать старые short filesystem ids в `ToolCatalog`. Канонический surface теперь состоит из typed variants: отдельные tools для чтения всего файла, диапазона строк, поиска, записи, patch, line replace и insert.

Это снижает когнитивный шум: новый разработчик и модель видят один основной filesystem API вместо двух параллельных naming schemes.
...
### 2. Не возвращать параллельные имена для одного действия

После 1.2.0 правило простое: один основной tool id на одно действие. Если нужен новый вариант, он должен добавлять явную семантику, а не быть коротким alias для уже существующего typed tool.
