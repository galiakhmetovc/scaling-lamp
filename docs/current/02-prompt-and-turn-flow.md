...
Порядок фиксирован и считается инвариантом проекта:
...
Идея простая: вместо того чтобы заставлять модель каждый раз заново “читать” всё состояние, runtime заранее упаковывает самые полезные факты.

## Откуда берутся системные тексты

`prompting.rs` загружает:
...
Оператор может запускать compaction вручную. Это важно знать: если summary ведёт себя странно, ищите не “автоархив”, а конкретную команду или вызов compaction.
