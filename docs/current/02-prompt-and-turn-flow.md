...
- `skill_install` — устанавливает целую директорию skill из workspace текущей session в `agent_home/skills/<name>/`. `source_dir` должен указывать на папку, а не на файл `SKILL.md`; папка обязана содержать валидный `SKILL.md` с frontmatter `name` и `description`. По умолчанию установленный skill сразу включается для текущей session.
