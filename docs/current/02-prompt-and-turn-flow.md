...
- session id и title;
- agent profile и путь к workspace агента;
- bounded operator context из `data_dir/USER.md`;
...
- workspace root;
- bounded excerpts сегодняшнего и вчерашнего SilverBullet daily journal, если `[knowledge]` это включает и space доступен;
- путь к SilverBullet session mirror, если оператор явно включил mirror и space доступен;
...
Идея простая: вместо того чтобы заставлять модель каждый раз заново “читать” runtime state, runtime заранее упаковывает самые полезные факты. `SessionHead` остаётся runtime orientation block. Schedule, subagents, A2A и mesh obligations уходят в отдельный `AutonomyState`, а недавняя tool/debug activity — в `RecentToolActivity` и debug surfaces.

`USER.md` — операторский контекст, а не agent profile. Production template хранит язык, timezone и правила интерпретации относительного времени. Runtime создаёт `data_dir/USER.md` при первом чтении, если файла ещё нет. Оператор может править его без пересборки.

SilverBullet journal context — bounded read-only excerpt в prompt. Перед изменением daily note агент всё равно должен прочитать целевой файл через SilverBullet MCP или filesystem tools.

SilverBullet session mirror — optional человекочитаемая page вида `p/teamd-session-<session_id>.md`. Mirror выключен по умолчанию, чтобы runtime не транслировал transient status/progress в базу заметок. Если оператор явно включает mirror, page помогает видеть plan snapshot, context summary, tool activity и artifacts в браузере, но runtime source of truth остаётся в PostgreSQL/transcripts/artifacts/tool-call ledger.
...
- `SYSTEM.md` из `workspaces/agents/<agent_id>/SYSTEM.md`;
- `AGENTS.md` из `workspaces/agents/<agent_id>/AGENTS.md`;
...
Built-in profiles дополнительно фиксируют два базовых правила поведения:

- самообучение должно быть явным: агент учитывает повторяющиеся ошибки tools, корректировки оператора и успешные workflows, но сохраняет устойчивые уроки только в inspectable местах;
- workspace должен оставаться чистым: временные файлы не создаются в корне проекта, durable outputs кладутся в canonical directories, случайный мусор удаляется до завершения работы.

Самообучение не является скрытой памятью модели. Если урок должен пережить текущий turn/session, агент обязан записать его через каноническую поверхность: knowledge/memory tools, SilverBullet Space note, artifact, документацию или согласованное изменение profile/skill prompts.

Встроенный template сейчас один: `default`. Он материализуется в workspace `default` profile и копируется при создании новых agent profiles. Fallback остаётся emergency-механизмом, а не нормальным скрытым prompt path.
...
- `skill_list` — показывает catalog для текущего agent profile из `workspaces/agents/<agent_id>/skills/`, включая activation mode и paths.
...
- `skill_install` — устанавливает целую директорию skill из workspace текущей session в `workspaces/agents/<agent_id>/skills/<name>/`. `source_dir` должен указывать на папку, а не на файл `SKILL.md`; папка обязана содержать валидный `SKILL.md` с frontmatter `name` и `description`. По умолчанию установленный skill сразу включается для текущей session.
...
## Память, KV, SilverBullet и `knowledge_*`

Эти слои намеренно не заменяют друг друга:

- Mem0 — semantic long-term memory для предпочтений, устойчивых фактов и коротких pointers на notes;
- KV — exact structured state в PostgreSQL table `kv_entries`, включая `agent_shared` для same-VM inter-agent coordination;
- SilverBullet — human-editable Markdown knowledge/work journal; optional session mirror включается отдельно;
- `knowledge_search`/`knowledge_read` — PostgreSQL full-text index по configured canonical docs/workspace knowledge roots, не произвольный filesystem search и не SilverBullet/Mem0/KV.

Если агент записал важную заметку в SilverBullet, он должен создать или обновить короткий Mem0 pointer: путь заметки, тема, краткое резюме, теги и зачем это важно. В следующей сессии runtime pre-turn recall может найти pointer, а модель уже через SilverBullet прочитает полную заметку.

Практическое правило выбора:

- нужно вспомнить предпочтение оператора, устойчивый факт или прошлый урок по смыслу — `memory_search`;
- нужно передать точное JSON-состояние между агентами на одной VM — `kv_get`/`kv_put` со scope `agent_shared`;
- нужно создать/прочитать человекочитаемую заметку, журнал, plan mirror или документацию — `silverbullet-space` skill и SilverBullet MCP/filesystem fallback;
- нужно найти документацию runtime/repo, справочник tools или operator docs в configured knowledge roots — `knowledge_search`/`knowledge_read`;
- нужно найти файл, созданный session/tool — artifact/session debug surfaces, а не `knowledge_*`.
...
10. Best-effort обновляет SilverBullet session mirror только если оператор явно включил `[knowledge].silverbullet_mirror_enabled = true` и space доступен.
...
- если refs не поместились, prompt показывает hidden count и подсказку искать context через `artifact_search` или читать его страницами через `artifact_read`.

Модель может управлять видимостью refs через canonical tools:

- `artifact_read` — читает bounded page payload и увеличивает `explicit_read_count`;
...
При успешной compaction runtime:

- пишет `ContextSummary` в store;
- увеличивает `compactifications` у session;
- добавляет system note в run;
- отправляет operator-visible `ContextStatus` event при live observer, например Telegram/TUI progress;
- best-effort обновляет SilverBullet session mirror только если mirror явно включён.

Auto-compaction threshold считается от usable context: `context_window_tokens * auto_compaction_trigger_ratio`. Оба значения можно переопределять через config/env.
