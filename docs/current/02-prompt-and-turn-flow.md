...
SilverBullet session mirror — человекочитаемая page вида `p/teamd-session-<session_id>.md`. Она помогает оператору видеть plan snapshot, context summary, tool activity и artifacts в браузере, но runtime source of truth остаётся в SQLite/transcripts/artifacts/tool-call ledger.
...
- KV — exact structured state в `state.sqlite`, включая `agent_shared` для same-VM inter-agent coordination;
- SilverBullet — human-editable Markdown knowledge/work journal и session mirror;
- `knowledge_search`/`knowledge_read` — SQLite FTS index по configured canonical docs/workspace knowledge roots, не произвольный filesystem search и не SilverBullet/Mem0/KV.
