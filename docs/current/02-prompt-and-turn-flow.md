...
Самообучение не является скрытой памятью модели. Если урок должен пережить текущий turn/session, агент обязан записать его через каноническую поверхность: knowledge/memory tools, Obsidian/MCP note, artifact, документацию или согласованное изменение profile/skill prompts.
