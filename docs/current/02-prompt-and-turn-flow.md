...
Модель может читать и менять session-scoped распределение бюджета через canonical tools:

- `prompt_budget_read` — показывает source policy, context window, auto-compaction basis, usable context tokens, проценты слоёв и target tokens по слоям.
- `prompt_budget_update` — сбрасывает policy или меняет проценты слоёв. После merge сумма процентов обязана быть `100`, иначе tool возвращает ошибку.

Сейчас это control-plane policy и debug visibility. Жёсткое обрезание каждого слоя по этим процентам должно оставаться отдельным шагом, чтобы не смешивать изменение prompt contract с большим refactor truncation логики.
