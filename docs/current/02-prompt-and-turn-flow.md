...
- путь к SilverBullet session mirror, если mirror включён и space доступен;
...
SilverBullet session mirror — человекочитаемая page вида `p/teamd-session-<session_id>.md`. Она помогает оператору видеть plan snapshot, context summary, tool activity и artifacts в браузере, но runtime source of truth остаётся в PostgreSQL/transcripts/artifacts/tool-call ledger.
...
- SilverBullet — human-editable Markdown knowledge/work journal и session mirror;
...
10. Best-effort обновляет SilverBullet session mirror, если `[knowledge].silverbullet_mirror_enabled = true` и space доступен.
...
- best-effort обновляет SilverBullet session mirror.
