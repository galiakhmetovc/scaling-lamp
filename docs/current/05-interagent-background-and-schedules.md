...
- `message_agent` ставит работу в очередь и возвращает ids дочерней ветки сразу;
- runtime создаёт запись в task registry, поэтому оператор видит работу через `/tasks`, `agentd session tasks <session_id>` или TUI `\задачи`;
- родительский агент обычно продолжает текущую работу, а не ждёт дочернюю session;
- `session_wait` используется только когда результат другого агента явно нужен прямо сейчас и wait ограничен timeout;
- прямое чтение дочерней session остаётся диагностическим способом посмотреть подробности.
...
Важно: `session_wait` не должен быть скрытым transport-level await. Если задача может продолжаться без ответа дочернего агента, агент должен продолжать и проверять task registry позже.
