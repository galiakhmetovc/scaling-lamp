...
- PostgreSQL-aware store retry wrapper вокруг runtime store операций;
...
- `knowledge_search` остаётся bounded PostgreSQL full-text index по configured roots, а не произвольным filesystem search; unreadable/stale/non-UTF8 files пропускаются при индексации;
