...
Быстрый путь для уже установленного сервера: собрать `agentd` локально и отправить готовый binary на production host без `cargo build` на сервере:
...
В private chat session создаётся автоматически, если `private_chat_auto_create_session = true`. В group/supergroup chat обычный текст без mention обрабатывается только от Telegram-пользователя с activated pairing. Для неактивированного пользователя при `group_require_mention = true` по-прежнему нужен mention/targeted command, иначе worker игнорирует сообщение.

Session-level команды требуют выбранную session. Если её нет, создайте `/new` или выберите `/use <session_id>`.

Agent-level команды `/agents`, `/agentuse` и `/newagent` не создают отдельный runtime path. Они только управляют Telegram chat binding: какой `Agent profile` использовать для новых sessions этого chat. Уже выбранная старая session остаётся привязанной к своему agent profile, пока не создать или выбрать другую session.
...
Если пользователь пишет в Telegram, пока текущая session уже выполняет turn, worker не запускает второй turn параллельно. Поведение задаётся per-chat настройкой `/queue` и видимо в `/status`.

Режимы:

- `coalesce` — default. Новые сообщения складываются в durable `session_inbox_events` и становятся доступны wakeup/background обработке после coalesce window. Минимум — `5s`, даже если в конфиге или команде указано меньше.
- `queue` — каждое сообщение сразу становится queued inbox event без coalesce-задержки.
- `reject` — новые сообщения отклоняются коротким ответом, пока turn выполняется.
- `restart` — worker запрашивает остановку активного turn и кладёт новое сообщение в inbox.
...
`/queue flush` делает queued Telegram-input доступным немедленно. `/queue clear` помечает queued Telegram-input как processed. Алиасы для ручного управления очередью: `/queue start` = `flush`, `/queue stop` = `clear`.
...
| `telegram.group_require_mention` | `true` | В группах требовать mention/targeted command от неактивированных пользователей; activated paired users могут писать обычным текстом без mention. |
| `telegram.default_autoapprove` | `true` | Включать auto-approve по умолчанию для Telegram-created sessions. |
| `telegram.inbound_queue_default_mode` | `coalesce` | Default-режим для входящих Telegram-сообщений во время активного turn: `reject`, `queue`, `coalesce`, `restart`. |
| `telegram.inbound_coalesce_window_ms` | `5000` | Coalesce window для `coalesce`; runtime валидирует минимум `5000`. |
