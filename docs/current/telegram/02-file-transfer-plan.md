...
Статус: v1 реализован для Bot API path.

Реализовано:

- приём Telegram `document` и `photo`;
- сохранение файла как `Artifact` текущей `Session`;
- передача агенту короткого user turn с `artifact_id`, metadata и caption;
...
- file-oriented tools `file_list`/`file_read` поверх artifacts;
- extractor’ы для PDF/DOCX/XLSX и других бинарных форматов.
...
sequenceDiagram
    ...
    Worker->>Runtime: execute chat turn with short artifact_id message
...
Часть generic tools уже есть:

- `deliver_file` — отправить существующий session artifact или workspace file пользователю через активную surface.

Остальные file-oriented aliases/tools остаются следующим слоем:
...
```text
deliver_file({ artifact_id: "...", target: "current_chat" })
deliver_file({ workspace_path: "reports/report.pdf", file_name: "report.pdf", caption: "Готовый отчёт" })
```
...
`status=queued` в результате `deliver_file` — это успешная постановка request в durable queue. Это не ошибка и не гарантия, что Bot API уже принял документ. Если Telegram delivery падает, worker переводит request в `failed`, сохраняет ошибку и пишет пользователю отдельное сообщение о неудачной доставке.
...
- runtime/tool `deliver_file` ставит request в `file_delivery_requests`;
- после финального ответа Telegram worker читает queued requests текущей session и отправляет каждый как `sendDocument`;
- successful delivery помечается `delivered`, ошибка — `failed` с текстом причины.
...
- файл из workspace, который `deliver_file` сначала сохраняет как session artifact;
- generated report/export.

Нельзя отправлять произвольный host path. `workspace_path` в `deliver_file` резолвится только относительно workspace текущей session и перед отправкой материализуется как artifact.
...
- generic `deliver_file` добавлен в automatic model-facing tool surface;
- `artifact_id` должен принадлежать текущей session;
- `workspace_path` должен быть relative path внутри workspace текущей session;
- Telegram implementation доставляет queued request через `sendDocument`;
- TUI/CLI остаются debug/operator views и не имеют отдельного provider loop.

### Step 4: better file reading and debug views

- добавить `file_list`/`file_info`/`file_read` как более понятные aliases поверх artifacts;
...
- показать queued/delivered/failed file deliveries в debug-view рядом с artifacts.
