...
- какие skills активировались автоматически или вручную перед turn;
- выполнялся ли pre-turn `MemoryRecall`, какой был query и сколько memory hits попало в prompt.
...
- Telegram temporary status и TUI active run status показывают runtime context events: active skills и memory recall summary.
...
Runtime также emits `ContextStatus` events перед основным provider loop:

- `label=skills`, если есть active skills, с перечислением `name` и mode `auto/manual`;
- `label=memory`, если выполнялся `MemoryRecall`, с query, количеством hits и короткими примерами.

Эти events не являются tool calls и не увеличивают счётчик tool calls. Они нужны только для operator visibility в Telegram/TUI/CLI/debug stream.
...
- через `skill_list`, если агенту нужно обнаружить capability;
- через `skill_read`, если агенту нужен полный bounded body `SKILL.md`;
- через `skill_enable`/`skill_disable`, если агенту нужно session-scoped включение/отключение;
- через документацию.
...
Полный текст skill доступен агенту через `skill_read`. Manual skills получают приоритет над automatic skills внутри budget. `skill_enable` и `skill_disable` меняют только `SessionSettings`, не редактируют skill files и не меняют templates.

Практическое правило для русскоязычной эксплуатации: `name` может оставаться stable English id, но `description` должен быть двуязычным. Automatic activation работает по token overlap, поэтому русский текст вроде “память”, “файл”, “телеграм”, “заметка”, “план”, “расписание” должен присутствовать в description, иначе агенту приходится угадывать skill через `skill_list`.
...
### Change C6d: добавить model-facing skill tools
...
- общий каталог skills больше не нужно вставлять в prompt;
- агент может сам обнаружить и прочитать нужный workflow;
- active skill prompt можно безопаснее ограничивать budget policy, потому что полный текст доступен через tool;
- активация навыка становится видимым persisted действием, а не скрытым prompt side effect.
