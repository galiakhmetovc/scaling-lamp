...
Для поиска runtime отправляет `POST /search` с `filters` и `top_k`, а не отдельные top-level `user_id`/`limit`. Пользовательские `filters` объединяются с entity filter выбранного scope. Mem0 здесь используется как semantic memory, а не как KV/state store: точные ключи, locks, counters и runtime-состояние должны жить во встроенном KV-слое `state.sqlite`.

### Built-in KV layer

У KV нет отдельной внешней инфраструктуры и отдельной config section. Он всегда хранится в `state.sqlite` в таблице `kv_entries` и доступен модели через built-in tools:
...
- KV/state store уже есть во встроенном `state.sqlite`; Mem0 хранит семантически извлекаемые memories, а не точные runtime-ключи.
