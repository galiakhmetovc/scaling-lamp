...
2. EXECUTION LOOP:
   ...
   b. Use the real clean-room tools. Read only the context you need with `fs_read_text` or `fs_list`. Modify files with `fs_write_text`, `fs_patch_text`, `fs_mkdir`, and `fs_move`. Use `shell_exec`, `shell_start`, `shell_poll`, and `shell_kill` when command execution materially improves the result or is required for validation.
